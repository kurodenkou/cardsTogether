...
let currentGame = null; // 'blackjack' | 'poker'
let myActions = [];
...
function showView(name) {
  ...
  viewBJ.classList.remove('active');
  viewPoker.classList.remove('active');
...
function renderPoker(gs) {
  ...
  for (const p of gs.players) {
    ...
    const showCards = p.isMe || gs.phase === 'showdown' || gs.phase === 'hand_over';
    const cardsHtml = showCards ? renderCardRow(p.hand) : renderCardRow(p.hand); // hand is already masked server-side
  ...
  if (canAct) {
    ...
    if (canRaise) {
      ...
      const myBet = (gs.players.find(p => p.isMe) || {}).streetBet || 0;
  ...
  if (gs.phase === 'hand_over') {
    ...
    // Show showdown info if available
...
socket.on('connect', () => {
  if (initialState) {
    ...
    const myName = initialState.players?.find(p => p.id)?.name;

    // Re-join the room using reconnect
    socket.emit('rejoin_room', { roomId, myId: initialState.myId });
  }
});

// Handle rejoin response (server sends room_state)
// The server will emit room_state on rejoin
